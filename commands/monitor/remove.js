...
module.exports = {
 ...
 let database = JSON.parse(fs.readFileSync("./link.json", "utf8"));
    ...
    if (!data) {
      return send(
        "<:no:957144549698986015> You do not have any site to monitor, use `.monitor` too add a website",
        message,
        "YELLOW"
      );
    }
    ...
    let embed = new discord.MessageEmbed()
      .setTitle("<a:loading:957145065900363829> Send The number of the link to remove")
      .setColor("BLUE")
    ...
    if (!repMsg) {
      msg.delete();
      return send(
        "<:no:957144549698986015> Cancelled The Process of deleting monitor website.",
        message,
        "RED"
      );
    }
    ...
    if (isNaN(repMsg.content)) {
      msg.delete();
      return send(
        "<:no:957144549698986015> Cancelled The Process of deleting monitor website due to **invalid digit**",
        message,
        "RED"
      );
    }
    ...
    if (!database[value].link[parseInt(repMsg.content) - 1]) {
      msg.delete();
      return send("<:no:957144549698986015> There is no link exist with this number.", message, "RED");
    }
    ...
    return send(
      "<a:yes:958151907979952159> Removed the website from monitoring, you can check website using `.stats`",
      message,
      "GREEN"
    );
  }
